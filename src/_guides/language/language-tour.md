...
layout: guide
title: "Dart 语言指南"
description: "所有主要的 Dart 语言特性概览"
short-title: 语言指南
---

假设您已经知道如何使用其他语言编程，本页面展示从变量和运算符到类和库，如何使用 Dart 的每个主要功能。

了解更多关于 Dart 的和辛苦，请参阅
[Dart 库指南](/guides/libraries/library-tour).

<div class="alert alert-info" markdown="1">
**注意：**
您在 [DartPad](/tools/dartpad) 中可以使用大部分特性。
</div>

了解有关语言特性更多详细信息，请参阅 [Dart 语言规范](/guides/language/spec)。


## Dart 编程入门

以下代码使用了许多 Dart 的基本特性：
...
{% endprettify %}
<a href="{{ site.custom.dartpad.direct-link }}" class='run-in-dartpad' target="_blank">打开 Dartpad</a>

以下是此程序使用到的特性，可应用于所有（或几乎所有）的 Dart 应用：
...
:   一种类型。另外其他的内置类型有 String、int 和 bool。
...
:   字符串插值：包含一个与字符串字面量等价的变量或表达式，更多详细信息，请参阅 [String](#strings)。

`main()`

:   一个特殊、*必须* 的顶级函数，应用程序执行的入口点，有关跟多信息，请参阅 [main() 函数](#the-main-function).
...
<div class="alert alert-info" markdown="1">
**注意:**
我们的代码风格遵循 [Dart 风格指南](/guides/language/effective-dart/style) 中的约定。
例如，我们使用双空格缩进。
</div>
...
在您了解 Dart 语言时，请留意这些事实和概念于心中：

-   您可以认为变量就是一个 *对象*，每个对象都是 *class* 的实例。甚至数字、函数和 `null` 都是对象。所有的 对象都继承自 [Object]({{site.dart_api}}/dart-core/Object-class.html) 类。

-   指定静态类型（如上例中的 `num`）
    来阐明您的意图，通过工具启用静态检查。
    （当您在调试代码时可能会注意到，没有指定类型的变量会得到一个特殊的类型：`dynamic`。）
    在 Dart 1.x 中，指定静态类型是可选的，然而，Dart 正在向完全类型安全语言的方向迁移。

-   在 [strong mode](/guides/language/sound-dart) 下，静态和运行时
    检查确保您的代码类型是安全的，可帮助您捕获开发中的 bug，而不是在运行时。在 Dart 1.x 中，Strong mode 是可选的，但在 [Dart 2.0](/dart-2.0) 中不可选。

-   Dart 在运行之前会解析您的所有代码，您可以提供建议给 Dart 以捕获错误或帮助代码运行得更快速，例如，通过使用类型或者编译时常量。

-   Dart 支持顶级函数（如 `main()`），以及绑定到类或对象（分别为 *静态方法* 和 *实例方法*）的函数。您还可以在函数中创建函数（*嵌套函数* 或者 *本地函数*）。

-   类似的，Dart 支持顶级*变量*，以及绑定到类或者对象（静态变量和实例变量）的变量。实例变量有时也称为字段或者属性。

-   与 Java 不同，Dart 没有 `public`、`protected` 和 `private` 关键字。如果一个标识符以一个下划线（\_）开头，那么它的库就是私有的。有关详细信息，请参阅[库与可见性](#libraries-and-visibility).

-   *标识符* 可以以字母或者 \_ 开头，后面跟上这些字符和数字的任何组合。

-   有时候问题在于是 *表达式* 还是 *语句*，因此我们要精确区分这两个词。

-   Dart 工具可以报告两种错误：警告和错误。
    警告只是指示您的代码可能无法正常运行，但不会阻止程序执行。错误可以是编译时或运行时。编译时错误会阻止代码执行；运行时错误导致代码执行时引发[异常](#exceptions)。

-   Dart 1.x 有两种***运行时模式***：
    生产和检查。我们建议您在检查模式下开发和调试，而在部署时切换到生产模式。
    *生产模式* 是 Dart 程序的默认运行时模式，
    其针对速度进行了优化。生产模式会忽略 [assert 语句](#assert) 和静态类型。
    *检查模式* 是一个开发人员友好的模式，可帮助您在运行时捕获一些类型错误。例如，您将一个非数字分配给一个声明为 `num` 的变量，则检查模式会抛出一个异常。
...
<sup>1</sup> 有上标 **1** 的单词是
are **内置标识符**。避免使用内置标识符作为标识符。
如果您尝试为一个类名或类型名使用内置标识符，则会发生编译时错误。

<sup>2</sup> 带上标 **2** 的单词是比较新的，
是在 Dart 1.0 版本之后添加的 **异步** 支持相关保留字。
您不能在任何标有 `async`、`async*`或 `sync*` 的函数体中使用 `async`、`await` 或 `yield` 作为标识符。
有关更多信息，请参阅[异步支持](#asynchrony-support)。

关键字表中的所有其他单词都是 **保留字**。您不能使用保留字作为标识符。
...
变量是引用的，名为 `name` 的变量包含了一个值为“Bob”的 String 对象引用。

### 默认值

未初始化的变量的初始值为 `null`。即使数值类型的变量初始也是 null，因为数字也是对象。
...
<div class="alert alert-info" markdown="1">
**注意：**
在生产模式下将忽略 `assert()` 调用。在检查模式下，
<code>assert(<em>condition</em>)</code> 
抛出一个异常，除非 *condition* 是 true。
有关详细信息，请参阅 [Assert](#assert) 小节。
</div>
...
添加类型是明确表达意图的一种方式。编译器和编辑器之类的工具可以使用这些类型，
通过提供代码完成和对错误和代码完成的早期警告来帮助您。

<div class="alert alert-info" markdown="1">
**注意：**
针对局部变量，此页面遵循使用 `var` 而不是类型注解的[风格指南建议](/guides/language/effective-dart/design#type-annotations)。
即使在 [strong mode](/dart-2.0) 下，也可以指定 `var` &mdash; 
分析器在可能的情况下推断类型。
</div>
...
如果您打算不更改变量，请使用 `final` 或 `const`，而不是使用 `var` 或特定类型。
Final 变量只能设值一次；const 变量是一个编译时常量（const 变量是隐式的 final）。
...
对于要成为编译时常量的变量，请使用 `const`。如果 const 变量处于类级别，
则将其标记为 `static const`。在变量声明的地方，将该值设置为一个编译时常数，
例如一个数字或字符串，一个常量变量或常量数字运算结果：
...
`const` 关键字不仅可用于声明常量变量。
您还可以使用它来创建常量_值_，以及声明_创建_常量值的构造函数。
任何变量都可以有一个常量值。
...
有关使用 `const` 创建常量值的更多信息，请参阅 [List](#lists)、[Map](#maps) 和 [Classe](#classes)。
...
- list （也成为 *array*）
- map
- rune （用于在字符串中表达 Unicode 字符）
- symbol

您可以使用字符串初始化任何这些特殊类型的对象。
例如，`'this is a string'` 是一个字符串，而 `true` 是一个布尔字符串。

因为 Dart 中的每个变量都指向一个对象 —— `class` 的一个实例 —— 通常可以使用构造函数来初始化变量。
一些内置类型有自己的构造函数。例如，您可以使用 `Map()` 构造函数创建一个 map，使用代码如 `new Map()`。
...
:   整型值，一般应在 -2<sup>53</sup> 到 2<sup>53</sup> 之间

[`double`]({{site.dart_api}}/dart-core/double-class.html)

:   64 位（双精度）浮点型数字，由 IEEE 754 标准规定

`int` 和 `double` 都是 [`num`]({{site.dart_api}}/dart-core/num-class.html) 的子类型。
num 类型包括了基本运算符，例如 +、-、/、和 \*，并且还可以在其他方法中找到 `abs()`、`ceil()` 和 `floor()`。
（位运算符，如 \>\>，定义在 `int` 类中。）
如果 num 和他的子类型中没有您想要的，[dart:math]({{site.dart_api}}/dart-math/dart-math-library.html) 库中可能有。

<div class="alert alert-warning" markdown="1">
**警告：**
-2<sup>53</sup> 至 2<sup>53</sup> 范围外的整数在 Dart 代码产生的 JavaScript 中的表现与
在 Dart VM 中运行相同 Dart 代码时的表现方式不同。
原因是 Dart 被指定为具有任意精度的整数，但 JavaScript 不是。
详见 [issue 1533](https://github.com/dart-lang/sdk/issues/1533)。
</div>

整数是没有小数点的数字。下面的例子定义了整数：
...
如果一个数字包含了小数，那么它将是一个 double。以下示例定义了 double：
